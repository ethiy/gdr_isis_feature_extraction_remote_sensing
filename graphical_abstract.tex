...
\begin{document}
    ...
    \begin{tikzpicture}
        ...
        \path (residual.south) + (0,-.2) node (residual_legend) {\tiny (c) Altim\'etrique};
        
        \path (residual.west) + (-.8,0) node (facet_graph) {\includestandalone[mode=buildnew, height=1cm]{building_graph}};
        \path (facet_graph |- residual_legend) node (facet_graph_legend) {\tiny (b) G\'eometrique};

        \path (facet_graph.west) + (-1,0) node (model) {\includestandalone[mode=buildnew, height=1cm]{building_model}};
        \path (model |- residual_legend) node (model_legend) {\tiny (a) Mod\`ele d'entr\'ee};
        
        \path (residual.east) + (.8,0) node (ortho_projection) {\includegraphics[height=1cm, angle=270]{images/orthoprojection}};
        \path (ortho_projection |- residual_legend) node (orthoprojection_legend) {\tiny (d) Optique};
        
        \draw[decorate, decoration={brace, amplitude=3pt}] (facet_graph.north west) -- (ortho_projection.east |- facet_graph.north) node[midway, yshift=5pt]{\tiny Attributs};

		\path (ortho_projection.east) + (1,0) node (errors) {\includestandalone[mode=buildnew, height=1cm]{building_errors}};
        \path (errors |- residual_legend) node (errors_legend) {\tiny (d) B\^atiments erron\'es:};
        \path (errors_legend.south) + (0,-.1) node[align=left] (errors_list) {\tiny {\color{red}$\blacksquare$ Erreur g\'eom\'etrique}.};
         \path (errors_list.south) node[align=left] (errors_list) {\tiny {\color{blue}$\blacksquare$ Erreur topologique}.};
